...
namespace Lazinator.CodeDescription
{
    ...
    public class LazinatorObjectDescription
    {
        ...
        public LazinatorObjectTypeEnum ObjectType { get; set; }
        ...
        public string ProtectedIfApplicable => (ObjectType == LazinatorObjectTypeEnum.Struct || GeneratingRefStruct || IsSealed) ? "" : "protected ";
        ...
        public bool IsClass => ObjectType == LazinatorObjectTypeEnum.Class && !GeneratingRefStruct;
        public bool IsStruct => !IsClass;
        public bool IsSealedOrStruct => IsSealed || ObjectType != LazinatorObjectTypeEnum.Class || GeneratingRefStruct;
        ...
        public long? PipelineStartTimeStamp { get; set; }
        ...
        public string NotAsync(string content) => AsyncTemplate.NotAsync(content);
        public string NotAsync_Begin => AsyncTemplate.NotAsync_Begin;
        public string NotAsync_End => AsyncTemplate.NotAsync_End;
        ...
        public string MaybeInIfNotAsync => AsyncTemplate.MaybeInIfNotAsync();
        ...
        public LazinatorObjectDescription(INamedTypeSymbol iLazinatorTypeSymbol, LazinatorImplementingTypeInfo compilation, LazinatorConfig? config, IDateTimeNow dateTimeNowProvider, bool suppressDate = false, long? pipelineStartTimeStamp = null)
        {
            ...
            PipelineStartTimeStamp = pipelineStartTimeStamp;
            ...
            {
                ObjectType = LazinatorObjectTypeEnum.Class;
            ...
            { // struct or ref struct
                ...
                //else
                ObjectType = LazinatorObjectTypeEnum.Struct;
            }
            ...
            IsSingleParent = InterfaceTypeSymbol.HasAttributeOfType<CloneSingleParentAttribute>();
        ...
        private void AppendDeclaration(CodeStringBuilder sb, string partialsuperclasses)
        {
            ...
            string theBeginning =
                    ...
                    {IIF(GeneratingRefStruct, $@"public ref partial struct { NameIncludingGenerics_RefStruct }")}{IIF(!GeneratingRefStruct, $@"{AccessibilityConverter.Convert(Accessibility)} { SealedKeyword }partial { (ObjectType == LazinatorObjectTypeEnum.Class ? ClassKeyword : "struct") } { NameIncludingGenerics } : {IIF(IsDerivedFromNonAbstractLazinator, () => BaseObjectName + ", ")}{ILazinatorStringWithoutQuestionMark}{IIF(AsyncLazinatorMemory, ", ILazinatorAsync")}")}
                    {{";
            ...
            if (BaseLazinatorObject == null)
                sb.AppendLine($@"{HideILazinatorProperty}public bool IsStruct => {(ObjectType == LazinatorObjectTypeEnum.Struct || GeneratingRefStruct ? "true" : "false")};
                        ");
                ...
                else if (IsAbstract && !GeneratingRefStruct)
                {
                    generalDefinitions = $@"
                        ...
                        {(ImplementsConvertFromBytesAfterHeader ? skipConvertFromBytesAfterHeaderString : $@"{ProtectedIfApplicable}abstract {TypeForLengths} ConvertFromBytesAfterHeader(IncludeChildrenMode includeChildrenMode, int serializedVersionNumber, ref int bytesSoFar);
                            ...
                            {ProtectedIfApplicable}abstract int ConvertFromBytesForChildLengths(ReadOnlySpan<byte> span, IncludeChildrenMode includeChildrenMode, int serializedVersionNumber, int indexOfFirstChild, ref int bytesSoFar);")}
...
")}
                ...
                else
                {
                    ...
                    generalDefinitions = $@"
                        ...
                        {{
                            [DebuggerStepThrough]
                            get => _IsDirty{IIF(!(ObjectType == LazinatorObjectTypeEnum.Struct || GeneratingRefStruct), "|| LazinatorMemoryStorage.Length == 0")};
                            [DebuggerStepThrough]
                            set
                            {{
                                if (_IsDirty != value)
                        ...
                        {{
                            ...
                            {{
                                LazinatorMemoryStorage = {MaybeAwaitWord}EncodeToNewBuffer{MaybeAsyncWord}(LazinatorSerializationOptions.Default);
                            }}
                            else
        ...
        private string GetCloneMethod()
        {
            ...
            clone.OnCompleteClone(this);")}
                        ...
                        {{
                            ...
                            {AppendCloneProperties()}{IIF(ObjectType == LazinatorObjectTypeEnum.Struct || GeneratingRefStruct, $@"
...
");
            ...
            for (int i = 0; i < withRecordedIndices.Count(); i++)
                if (withRecordedIndices[i].DerivationKeyword != "override ")
                    sb.AppendLine($"        {ProtectedIfApplicable}{TypeForLengths} {withRecordedIndices[i].BackingFieldByteIndex};");
            ...
            if (lastPropertyToIndex != null)
            {
                ...
                else
                {
                    ...
                    { // not sealed or struct etc.
                        bool mustInitialize = ((ObjectType != LazinatorObjectTypeEnum.Struct && !GeneratingRefStruct) && (lastPropertyToIndex.PropertyType == LazinatorPropertyType.OpenGenericParameter));  // initialization suppresses warning in case the open generic is never closed
                        sb.AppendLine(
        ...
        private void AppendConversionSectionStart(CodeStringBuilder sb)
        {
            ...
            else if (ObjectType == LazinatorObjectTypeEnum.Class && !GeneratingRefStruct)
...
");
        ...
        private void AppendUpdateDeserializedChildren(CodeStringBuilder sb)
        {
            ...
            foreach (var property in PropertiesToDefineThisLevel.Where(x => !x.IsPrimitive && !x.IsNonLazinatorType && x.PlaceholderMemoryWriteMethod == null))
            {
                ...
                bool asyncConditionForUpdateStoredBuffer = false; // property.AsyncWithinAsync
                sb.AppendLine(new ConditionalCodeGenerator(property.GetNonNullCheck(true),
$@"{MaybeAwaitWordConditional(asyncConditionForUpdateStoredBuffer)}{property.PropertyName}{property.NullForgiveness}{IIF(property.PropertyType == LazinatorPropertyType.LazinatorStructNullable || (property.IsDefinitelyStruct && property.Nullable), ".Value")}.UpdateStoredBuffer{MaybeAsyncWordConditional(asyncConditionForUpdateStoredBuffer)}({writerReference}, startPosition + {property.BackingFieldByteIndex}, {property.BackingFieldByteLength}, IncludeChildrenMode.IncludeAllChildren, true);").ToString());
            }
...
$@"_{propertyName} = ({property.AppropriatelyQualifiedTypeName}) CloneOrChange_{property.AppropriatelyQualifiedTypeNameEncodable}({property.BackingFieldWithPossibleValueDereferenceWithPossibleException}, l => l.RemoveBufferInHierarchy(), true);").ToString());
        ...
        private void AppendWritePropertiesIntoBuffer(CodeStringBuilder sb)
        {
            ...
            sb.AppendLine(
                    ...
                    ")}{(CanNeverHaveChildren ? "" : $@"writer.Write((byte)options.IncludeChildrenMode);")}");
            ...
            {
                sb.AppendLine($@"OnPropertiesWritten(options.UpdateStoredBuffer);");
            }
...
");
        ...
        public string GetLengthsCalculation(bool readVersion, bool optionsVariable)
        {
            var properties = PropertiesIncludingInherited_OpeningClosedGenerics.Where(x => !x.IsPrimitive && x.BytesUsedForLength() > 0).Select(x => (x.InclusionConditionalHelper(readVersion, optionsVariable).ToString(), x.BytesUsedForLength()));
...
totalChildrenBytes = base.ConvertFromBytesForChildLengths(span, OriginalIncludeChildrenMode, serializedVersionNumber, indexOfFirstChild, ref bytesSoFar);")}
        ...
        private string GetConstructors(CodeStringBuilder sb)
        {
            ...
            string constructors =
                        ...
                        {{{IIF(!inheritFromBaseType, $@"
                            ...
                            OriginalIncludeChildrenMode = originalIncludeChildrenMode;
                            LazinatorParents = new LazinatorParentsCollection(parent{IIF(!IsSingleParent, ", null")});
        ...
        private string GetDescendantDirtinessChecks(bool usePastTense)
        {
            ...
            if (ObjectType == LazinatorObjectTypeEnum.Struct || GeneratingRefStruct)
            {
...
");
        ...
        private string GetFileHeader(string hash, string primaryNamespace, List<string> otherNamespaces)
        {
            ...
            header.Append($@"//{@hash}
                //------------------------------------------------------------------------------
                // <auto-generated>
                //     This code was generated by the Lazinator tool, version {LazinatorVersionInfo.LazinatorVersionString}{(SuppressDate ? "" : $", on {DateTimeNowProvider.Value.ToString("yyyy/MM/dd hh:mm:ss.fff tt")}{(PipelineStartTimeStamp != null ? $" (ID: {PipelineStartTimeStamp})" : "")}")}.
                //
