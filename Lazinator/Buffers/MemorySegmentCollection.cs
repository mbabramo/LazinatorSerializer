...
namespace Lazinator.Buffers
{
    public partial class MemorySegmentCollection : MemoryChunkCollection, IMemorySegmentCollection
    {
        ...
        {
            if (recycle)
                Segments = new List<MemoryBlockSegmentByID>();
        }
        ...
        public void ExtendMemoryChunkReferencesList(MemoryChunkReference newSegment, bool extendEarlierReferencesForSameChunk)
        {
            if (Segments.Any())
            {
                ...
                {
                    last = new MemoryBlockSegmentByID(last.MemoryBlockID, last.Offset, last.Length + newSegment.FinalLength);
                    Segments[Segments.Count - 1] = last;
                    RecycledTotalLength += newSegment.FinalLength;
                    return;
                }
            }
            Segments.Add(new MemoryBlockSegmentByID(newSegment.MemoryBlockID, newSegment.AdditionalOffset, newSegment.FinalLength));
            RecycledTotalLength += newSegment.FinalLength;
        }
