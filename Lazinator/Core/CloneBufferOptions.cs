...
namespace Lazinator.Core
{
    ...
    public enum CloneBufferOptions
    {
        ...
        LinkedBuffer,
        /// <summary>
        /// A new buffer is created for the clone, and the original is updated to use the same buffer. The two objects are still independent, so if one is mutated and then serialized, a new buffer will then be created. Disposing the memory buffer makes both objects unusable. Note that disposing need not be done manually, as garbage collection will also eventually return memory to the pool, but manual disposing improves performance.
        /// </summary>
        SharedBuffer,
        /// <summary>
