...
namespace Lazinator.Support
{
    ...
    public static class Farmhash
    {
        ...
        public static unsafe uint Hash32(ReadOnlySpan<byte> s)
        {
            if (s.IsEmpty)
                return 0;
            ref byte x = ref MemoryMarshal.GetReference(s);
            fixed (byte* buf = &x)
            {
                return Hash32(buf, (uint) s.Length);
            }
        }

        public static unsafe ulong Hash64(ReadOnlySpan<byte> s)
        {
            if (s.IsEmpty)
                return 0;
            ref byte x = ref MemoryMarshal.GetReference(s);
            fixed (byte* buf = &x)
            {
                return Hash64(buf, (uint)s.Length);
            }
        }
