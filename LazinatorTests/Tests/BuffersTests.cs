...
namespace LazinatorTests.Tests
{
    public class BuffersTests : SerializationDeserializationTestBase
    {
        ...
        public void BufferOfCloneIsIndependent()
        {
            ...
            e.LazinatorMemoryStorage.SingleMemoryBlock.Should().Be(e.MyChild1.LazinatorMemoryStorage.SingleMemoryBlock);
            e.MyChild1.SerializeLazinator();
            e.LazinatorMemoryStorage.SingleMemoryBlock.Should().NotBe(e.MyChild1.LazinatorMemoryStorage.SingleMemoryBlock);
        ...
        public void ObjectDisposedExceptionThrownOnItemRemovedFromHierarchy()
        {
            ...
            Action a = () => { var x = e.MyChild1.LazinatorMemoryStorage.SingleMemoryBlock.ReadOnlyMemory; }; // note that error occurs only when looking at underlying memory
        ...
        public void UpdateBufferForDeserialized_LazinatorList_Struct()
        {
            ...
            c.MyInt = -234;
            UpdateStoredBufferFromExisting(c);
            var storageOverall = c.LazinatorMemoryStorage.SingleMemoryBlock;
            var storageItem = c.MyStructList[0].LazinatorMemoryStorage.SingleMemoryBlock;
        ...
        private static void ConfirmBuffersUpdateInTandem(ILazinator itemToUpdate)
        {
            ...
            var initialMemoryBlock = itemToUpdate.LazinatorMemoryStorage.SingleMemoryBlock;
            var allocationID = initialMemoryBlock.AllocationID;
            itemToUpdate.ForEachLazinator(x => 
            {
                if (x.LazinatorMemoryStorage.IsEmpty == false)
                {
                    MemoryBlock c = x.LazinatorMemoryStorage.SingleMemoryBlock;
                    if (c != null)
