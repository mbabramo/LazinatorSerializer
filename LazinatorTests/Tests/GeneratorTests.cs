  ...
  ""Comment"": ""This is a change that should trigger regeneration of code.""
        ...
        public Task SourceGeneratorCanMatchExistingLazinatorFiles(string projectName)
        {
            ...
            for (int i = 0; i < orderedGeneratedSources.Count(); i++)
            {
                ...
                if (!orderedGeneratedSources[i].path.Contains("NullableContextEnabled")) // We are periodically getting different info about whether we're in a nullable enabled context; must still figure it out, but it doesn't relate to this test
                {
                    if (matchesIgnoringGenerationInfo == false)
                    {
                        throw new Exception($"Generated code index {i} does not match original code for {orderedGeneratedSources[i].path}");
                    }
                }
            }
