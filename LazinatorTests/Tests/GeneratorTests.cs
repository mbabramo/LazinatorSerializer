  ...
  ""Comment"": ""This is a change that should trigger regeneration of code.""
        ...
        public Task SourceGeneratorCanMatchExistingLazinatorFiles(string projectName)
        {
            ...
            for (int i = 0; i < orderedGeneratedSources.Count(); i++)
            {
                ...
                if (!matchesIgnoringGenerationInfo)
                {
                    System.IO.File.WriteAllText(orderedGeneratedSources[i].path, orderedGeneratedSources[i].text);
                    throw new Exception($"Generated code does not match original {i}: {orderedGeneratedSources[i].text}"); // DEBUG -- trying to debug occasional error.
                }
