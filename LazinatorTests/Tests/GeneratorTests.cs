...
namespace LazinatorTests.Tests
{
  ...
  ""Comment"": ""This is a change that should trigger regeneration of code.""
        ...
        private static GeneratorExecutionResult RunGeneratorForSimpleLazinator(IDateTimeNow dateTimeNowProvider)
        {
            List<(string path, string text)> sources = GetSourcesForSimpleLazinator();
            var executionResult = ExecuteGeneration(sources, null, dateTimeNowProvider);
...
namespace MyCode
{
    public partial class MyExample : IMyExample
    {
        ...
        private static GeneratorExecutionResult ExecuteGeneration(IEnumerable<(string path, string text)> sources, IEnumerable<(string path, string text)> additionalTexts, IDateTimeNow dateTimeNowProvider)
        {
            ...
            // Create references for assemblies we required -- note that this is building on the existing project
            ...
            return new GeneratorExecutionResult(generator, (CSharpCompilation) outputCompilation, driver);
        }
