...
namespace Lazinator.CodeDescription
{
    public class PropertyDescription
    {
        ...
        public PropertyDescription(IPropertySymbol propertySymbol, ObjectDescription container)
        {
        ...
        private void SetTypeNameAndPropertyType(ITypeSymbol typeSymbol)
        {
            ...
            bool isSelfSerializable = typeSymbol.Interfaces.Any(x => x.Name == "ILazinator");
        ...
        private void SetTypeNameWithInnerProperties(string name, ImmutableArray<ITypeSymbol> typeArguments)
        {
            ...
            TypeName = name + "<" + string.Join(", ", InnerProperties.Select(x => x.TypeName)) + ">";
        ...
        private void SetSupportedCollectionTypeNameAndPropertyType(INamedTypeSymbol t, string nameWithoutArity)
        {
            ...
            InnerProperties = t.TypeArguments
                .Select(x => new PropertyDescription(x, Container)).ToList();
            TypeName = nameWithoutArity + "<" + string.Join(", ", InnerProperties.Select(x => x.TypeName)) + ">";
            TypeNameEncodable = EncodableTypeName(t);
...
");
        ...
        {{
            ...
            {{
                ...
                IsDirty = true;
                _{PropertyName} = new ReadOnlyMemory<byte>(MemoryMarshal.Cast<{innerType}, byte>(value).ToArray());
                _{PropertyName}_Accessed = true;
            }}
        }}
        internal bool _{PropertyName}_Accessed;
...
");
        ...
        private void AppendReadOnlySpan_SerializeExistingBuffer(CodeStringBuilder sb)
        {
            ...
            string innerType = InnerProperties[0].TypeName;
        ...
        public void AppendPropertyReadInSupportedCollectionString(CodeStringBuilder sb)
        {
            if (IsPrimitive)
                ...
                sb.AppendLine(
                        ...
                        " + CreateConditionalForSingleLine(ReadVersionNumberConditional, "bytesSoFar = span.ToInt32(ref bytesSoFar) + bytesSoFar;"));
        }
                ...
                else
                {
                    ...
                    sb.AppendLine(
                        ...
                        {{
                            ...
                            WriteChildWithLength(writer, _{PropertyName}, includeChildrenMode, _{PropertyName}_Accessed, () => GetChildSlice(serializedBytesCopy, byteIndexCopy, byteLengthCopy), verifyCleanness);
                        }}");
                }
            }
            else
