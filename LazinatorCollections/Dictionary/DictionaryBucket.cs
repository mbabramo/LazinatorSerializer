...
namespace LazinatorCollections.Dictionary
{
    public partial class DictionaryBucket<TKey, TValue> : IDictionaryBucket<TKey, TValue> where TKey : ILazinator where TValue : ILazinator
    {
        ...
        private int GetKeyIndex(TKey key, uint? binaryHashOfKey)
        {
            if (!Initialized)
                return -1;
            if (_lastSearchRemembered && (_lastKeySearched.LazinatorMemoryStorage.IsEmpty || _lastKeySearched.LazinatorMemoryStorage.Disposed == false) && _lastKeySearched.Equals(key))
                return _lastResult;
