...
namespace Lazinator.CollectionsTests
{
    public class ListableTests
    {
        public enum ListFactoryToUse
        {
            ...
            AvlListWithAvlIndexableListTreeCompounded,
            AvlSortedList,
            ...
            AvlSortedListGiantAllowDuplicates,
        }
        ...
        static ContainerFactory GetListFactory(ListFactoryToUse l)
        {
            switch (l)
            {
                ...
                case ListFactoryToUse.AvlListWithAvlIndexableListTreeCompounded:
                    return new ContainerFactory(new List<ContainerLevel>()
                    {
                        new ContainerLevel(ContainerType.AvlList),
                        new ContainerLevel(ContainerType.AvlIndexableListTree, false),
                        new ContainerLevel(ContainerType.AvlIndexableListTree, false, 30),
                        new ContainerLevel(ContainerType.AvlIndexableListTree, false, 10),
                        new ContainerLevel(ContainerType.LazinatorList, false, 3)
                    });
                ...
                case ListFactoryToUse.AvlSortedListGiantAllowDuplicates:
                    return new ContainerFactory(new List<ContainerLevel>()
                    {
                        new ContainerLevel(ContainerType.AvlSortedList, true),
                        new ContainerLevel(ContainerType.AvlSortedIndexableListTree, true),
        ...
        [InlineData(ListFactoryToUse.AvlListWithAvlIndexableListTreeCompounded)]
        ...
        public void Listable_AddingAtEnd(ListFactoryToUse listFactoryToUse)
        {
            ...
            int numItems = (listFactoryToUse == ListFactoryToUse.UnbalancedAvlList || listFactoryToUse == ListFactoryToUse.UnbalancedAvlSortedList) ? 20 : 300;
            for (int i = 0; i < numItems; i++)
            {
                l.Add(i);
                l.LongCount.Should().Be(i + 1);
            }
            // check accesing by index
            for (int i = 0; i < numItems; i++)
            {
                l.GetAtIndex(i).WrappedValue.Should().Be(i);
            }
            // check enumeration
            var result = l.AsEnumerable().Select(x => x.WrappedValue).ToList();
        ...
        [InlineData(ListFactoryToUse.AvlSortedListWithAvlSortedIndexableListTree)]
        [InlineData(ListFactoryToUse.AvlSortedListWithAvlSortedIndexableListTreeAllowDuplicates)]
        public void Listable_AddingAtBeginning_IntInClass(ListFactoryToUse listFactoryToUse)
        {
            var factory = GetListFactory(listFactoryToUse);
            ILazinatorListable<IntInClass> l = factory.CreatePossiblySortedLazinatorListable<IntInClass>();
            int numItems = (listFactoryToUse == ListFactoryToUse.UnbalancedAvlList || listFactoryToUse == ListFactoryToUse.UnbalancedAvlSortedList) ? 20 : 300;
            for (int i = 0; i < numItems; i++)
                l.InsertAtIndex(0, i);
            l.Any().Should().BeTrue();
        ...
        [InlineData(ListFactoryToUse.AvlListWithAvlIndexableListTreeCompounded, 200, 1)]
        ...
        [InlineData(ListFactoryToUse.AvlListWithAvlIndexableListTreeCompounded, 15, 20)]
