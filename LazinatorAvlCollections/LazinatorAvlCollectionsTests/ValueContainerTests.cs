...
using LazinatorAvlCollectionsTests.LazinatorObjects;
...
namespace Lazinator.CollectionsTests
{
    ...
    public class ValueContainerTests_IntInClass : ValueContainerTests<IntInClass>
    {
        [Theory]
        [InlineData(ValueContainerToUse.AvlTree, false, 50, 50)]
        [InlineData(ValueContainerToUse.AvlIndexableTree, false, 50, 50)]
        ...
        [InlineData(ValueContainerToUse.LazinatorList, true, 50, 50)]
        [InlineData(ValueContainerToUse.LazinatorLinkedList, true, 50, 50)]
        public void VerifyIntInClassContainer(ValueContainerToUse containerType, bool allowDuplicates, int numRepetitions, int numInstructions) => VerifyValueContainerHelper(containerType, allowDuplicates, numRepetitions, numInstructions);
        ...
        public void ValueContainer_SplitOff(ValueContainerToUse containerType)
        {
            IValueContainer<IntInClass> container = GetValueContainer(containerType, false);
            const int numItems = 200;
            for (int i = 0; i < numItems; i++)
                container.InsertOrReplace(i, Comparer<IntInClass>.Default);
            var splitOff = container.SplitOff();
            (container.Count() + splitOff.Count()).Should().Be(numItems);
            if (splitOff.First() > container.First())
            {
                foreach (var x in splitOff)
                    container.InsertOrReplace(x, Comparer<IntInClass>.Default);
            }
            else
            {
                foreach (var x in container)
                    splitOff.InsertOrReplace(x, Comparer<IntInClass>.Default);
                container = splitOff;
            }
            container.Select(x => x.WrappedValue).SequenceEqual(Enumerable.Range(0, numItems)).Should().BeTrue();
        }

        public override IntInClass GetRandomValue()
        {
            return ran.Next(100);
        }
    }
    ...
    public abstract class ValueContainerTests<T> where T : ILazinator, IComparable<T>
    {
        ...
        public IValueContainer<T> GetValueContainer(ValueContainerToUse containerType, bool allowDuplicates)
        {
            ...
            switch (containerType)
            {
                case ValueContainerToUse.AvlTree:
                    return new AvlTree<T>(allowDuplicates, false, true);
                case ValueContainerToUse.AvlIndexableTree:
                    return new AvlIndexableTree<T>(allowDuplicates, false, true);
                case ValueContainerToUse.AvlSortedTree:
                    return new AvlSortedTree<T>(allowDuplicates, false, true);
                case ValueContainerToUse.AvlSortedIndexableTree:
                    return new AvlSortedIndexableTree<T>(allowDuplicates, false, true);
                ...
                case ValueContainerToUse.AvlListTreeCompoundedFollowedByLinkedList:
                    ...
                    {
                        new ContainerLevel(ContainerType.AvlListTree, allowDuplicates, long.MaxValue, false),
                        new ContainerLevel(ContainerType.AvlListTree, allowDuplicates, 3, false),
                        new ContainerLevel(ContainerType.AvlListTree, allowDuplicates, 3, false),
        ...
        public void VerifyValueContainerHelper(ValueContainerToUse containerType, bool allowDuplicates, int numRepetitions, int numInstructions)
        {
            ...
            bool verifyAfterEachInstruction = false;
            for (int rep = 0; rep < numRepetitions; rep++)
            {
                ...
                for (int i = 0; i < numInstructions; i++)
                {
                    ...
                    if (verifyAfterEachInstruction)
                    {
                        VerifyMultilevelReporting(container);
                        VerifyFirstAndLast(list, container);
        ...
        private static void VerifyMultilevelReporting(IValueContainer<T> container)
        {
            if (container is AvlListTree<T> listTree)
            {
                var underlyingTree = listTree.UnderlyingTree;
                underlyingTree.MultilevelReporterParent.Should().Be(listTree);
                foreach (IMultivalueContainer<T> childContainer in underlyingTree.AsEnumerable())
                {
                    ((IMultilevelReporter)childContainer).MultilevelReporterParent.Should().Be(underlyingTree);
                    VerifyMultilevelReporting(childContainer);
                }
            }
        }

        private static void VerifyFirstAndLast(List<T> list, IValueContainer<T> container)
        {
            // This is an extra check, useful since we are caching first and last items, and mistakes may not cause problems when we verify the entire list, since that doesn't use the cache
            if (list.Any())
            {
                container.First().Equals(list.First()).Should().BeTrue();
                container.Last().Equals(list.Last()).Should().BeTrue();
            }
        }
